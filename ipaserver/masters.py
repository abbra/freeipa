...
def get_ca_service():
    ...
    Returns:
        str: 'ipacta' for Python CA or 'dogtag' for traditional PKI
             (default)
    """
    try:
...
def get_ca_systemd_service_name():
    ...
    Returns:
        str: 'ipacta' if using Python CA, 'pki-tomcatd' if using Java PKI
    """
...
def _get_ca_service_definition():
    ...
    if ca_backend == 'ipacta':
        return service_definition('ipacta', 50, 'CA')
...
def _get_kra_service_definition():
    ...
    ca_backend = get_ca_service()
    if ca_backend == 'ipacta':
        return service_definition('ipacta', 51, 'KRA')
