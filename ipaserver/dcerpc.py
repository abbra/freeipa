...
class TrustDomainInstance:
    ...
    def retrieve_anonymously(self, remote_host,
        ...
        if search_result:
            if is_ipa:
                try:
                    _objtype, res = conn.search_s(search_result, _ldap.SCOPE_SUBTREE,
                        '(&(objectclass=posixaccount)(objectclass=ipaNTUserAttrs)(uid=admin))',
                    ...
                    if 'ipaNTSecurityIdentifier' in res:
                        # Parse sid string to see if it is really in a SID format
                        try:
                            test_sid = security.dom_sid(res['ipaNTSecurityIdentifier'][0])
                        except TypeError:
                            raise errors.ValidationError(name='sid',
                                                         error=_('SID is not valid'))
