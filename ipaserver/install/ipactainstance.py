...
"""
Installation module for ipacta Python CA backend.

``IpactaInstance`` is an IPA ``service.Service`` subclass that drives the
installation and lifecycle of the ipacta CA.  It uses composition with
focused helper classes provided by the ``ipacta.install`` package:
...
translates PKI key names into the ipacta-native values stored in
``IpactaConfig``.
"""
...
# immutability rules) so that ipacta/config.py stays free of those
# details.  The build() classmethod returns a merged ConfigParser; the
# extract_ipacta_params() classmethod translates the PKI key names into
# the ipacta-native names accepted by IpactaConfig.from_install_params().
# ---------------------------------------------------------------------------
...
class _PKIConfigBuilder:
    ...
    # Maps (pki_section, pki_key) → (ipacta_param_name, default_value).
    # Only the keys that ipacta actually reads are listed here.
    _PKI_TO_IPACTA = {
    ...
    @classmethod
    def extract_ipacta_params(cls, pki_config):
        """Translate PKI config values to ipacta-native param names.
        ...
        Returns:
            ...
            ``IpactaConfig.from_install_params()``.
        """
        result = {}
        for (section, pki_key), (
            ipacta_name,
            ...
            result[ipacta_name] = pki_config.get(
...
class IpactaInstance(service.Service):
    """Installation and configuration module for ipacta Python CA backend.

    Uses composition with helper classes from ``ipacta.install``.
    ...
    def configure_certmonger_renewal_helpers():
        ...
        _logger.debug("Configuring certmonger helpers for ipacta HTTPS")
        ...
        for suffix, args in [
            ...
            if not path:
                ...
                _logger.debug("Registering certmonger CA helper: %s", name)
                iface.add_known_ca(
                    name,
                    command,
    ...
    def __init__(
        ...
        # the translated IpactaConfig used by everything else.
        ...
        if self.realm and host_name:
            ...
            self.config = IpactaConfig.from_install_params(
                realm=realm,
                host=host_name,
                basedn=api.env.basedn,
                random_serial_numbers=random_serial_numbers,
                **_PKIConfigBuilder.extract_ipacta_params(self.pki_config),
            )
        else:
        ...
        if self.tokenname:
            ...
            logger.debug(
                "HSM configuration validated: token=%s", self.tokenname
            )
        ...
        self.ca_key_path = Path(paths.IPACTA_SIGNING_KEY)
        self.ca_cert_working = self.ipaca_certs_dir / "ca.crt"

        self.audit_log_dir = Path(paths.IPACTA_LOG_DIR)
    ...
    def is_crlgen_enabled(self):
        ...
        For ipacta, CRL generation is always enabled on the local instance.
        """
    ...
    def _configure_ldap_access(self):
        ...
        try:
            ...
            logger.debug("ipacasrv sysaccount already exists: %s", ipacasrv_dn)
        except errors.NotFound:
            logger.debug("Creating ipacasrv sysaccount at %s", ipacasrv_dn)
        ...
        if ipaca_info:
            ...
            logger.debug("Creating LDAPI autobind mapping for ipaca ")

            try:
                autobind_entry = ldap.get_entry(autobind_dn)
                logger.debug("Autobind entry already exists: %s", autobind_dn)
                ...
                logger.debug("Created autobind entry: %s", autobind_dn)
        ...
        try:
            ...
            if not has_dogtag_aci:
                ...
                logger.debug(
                    "Adding ACI for ipacasrv access to Dogtag root suffix %s",
                    dogtag_suffix_dn,
                )
            ...
            logger.debug(
                "Dogtag suffix %s not found - ACI will be added when Dogtag "
        ...
        try:
            ...
            from ipacta.subca import SubCAManager
            ...
            logger.warning(
                "Failed to initialize sub-CA schema (may already exist): %s", e
            )
    ...
    def _load_external_cert(self, cert_files, ca_subject):
        ...
        ``ipacta.install.certs`` does not need to import from ipaserver.
        """
        from ipaserver.install import installutils
    ...
    def _create_default_caacl(self):
        ...
        try:
            ...
            logger.debug("Error checking for existing CA ACLs: %s", e)
        ...
        try:
            ...
            logger.error("Failed to create default CA ACL: %s", e)
            raise
    ...
    def _ensure_global_config(self):
        """Ensure global ipacta config is initialized.
        ...
        try:
            ...
            try:
                ...
                if not config_path.exists():
                    logger.debug(
                        "ipacta config file does not exist, "
                ...
                logger.debug(
                    "Initialized ipacta global config for healthcheck"
                )
                return True
            except Exception as e:
                logger.debug("Could not initialize global config: %s", e)
                return False
    ...
    def hsm_enabled(self):
        ...
        try:
            ...
            logger.debug("Could not check HSM status: %s", e)
            return False
    ...
    def token_name(self):
        ...
        try:
            ...
            logger.debug("Could not get HSM token name: %s", e)
            return None
    ...
    def enable_client_auth_to_db(self):
        """Enable client auth to LDAP database (no-op for ipacta)."""
        logger.debug("enable_client_auth_to_db: No-op for ipacta")
    ...
    def stop_tracking_certificates(self):
        ...
        if not services.knownservices.dbus.is_running():
            try:
                ...
                logger.warning("Could not start dbus: %s", e)
                return

        try:
            cmonger.start()
        except Exception as e:
            logger.warning("Could not start certmonger: %s", e)
            return
        ...
        try:
            ...
            logger.debug("RA agent certificate not tracked or error: %s", e)

        try:
            certmonger.stop_tracking(certfile=paths.HTTPD_CERT_FILE)
            logger.debug("Stopped tracking HTTPS certificate")
        except RuntimeError as e:
            logger.debug("HTTPS certificate not tracked or error: %s", e)
    ...
    def create_instance(
        ...
        """Create and configure ipacta Python CA instance.
        ...
        """
        logger.debug("Creating ipacta Python CA instance")
    ...
    def uninstall(self):
        ...
        try:
            ipautil.run(["systemctl", "disable", "ipacta.service"])
            logger.debug("ipacta service disabled")
        except Exception as e:
            logger.debug("Failed to disable ipacta service: %s", e)

        # Stop service
        try:
            ipautil.run(["systemctl", "stop", "ipacta.service"])
            logger.debug("ipacta service stopped")
        except Exception as e:
            logger.debug("Failed to stop ipacta service: %s", e)

        # Mask the service to prevent any activation
        try:
            ipautil.run(["systemctl", "mask", "ipacta.service"])
            logger.debug("ipacta service masked")
        except Exception as e:
            logger.debug("Failed to mask ipacta service: %s", e)

        # Remove service file
        service_path = Path("/etc/systemd/system/ipacta.service")
        if service_path.exists():
            service_path.unlink()
            logger.debug("ipacta service file removed")

        # Unmask after removing service file
        try:
            ipautil.run(["systemctl", "unmask", "ipacta.service"])
            logger.debug("ipacta service unmasked")
        except Exception as e:
            logger.debug("Failed to unmask ipacta service: %s", e)

        try:
            ipautil.run(["systemctl", "daemon-reload"])
            logger.debug("systemd reloaded")
        except Exception as e:
            logger.debug("Failed to reload systemd: %s", e)

        try:
            ipautil.run(["systemctl", "reset-failed", "ipacta.service"])
            logger.debug("ipacta service state reset")
        except Exception as e:
            logger.debug("Failed to reset ipacta service: %s", e)

        # Remove configuration
        config_path = Path(paths.IPACTA_CONF)
        if config_path.exists():
            config_path.unlink()
            logger.debug("ipacta configuration removed")
        ...
        if self.ipaca_dir.exists():
            try:
                ...
                logger.warning("Failed to remove %s: %s", self.ipaca_dir, e)
