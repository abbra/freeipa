...
class IPAThinCAInstance(service.Service):
    ...
    # with AttributeError when ipathinca is the active CA backend.
    ajp_secret = None
    hsm_sstore = None
    ...
    def create_certstore_passwdfile(self):
        """ipathinca has no Dogtag NSSDB — nothing to do."""

    # ------------------------------------------------------------------
    # Dogtag-compatible stubs
    #
    # upgrade.py and other callers written for Dogtag call these on
    # whatever object CAInstance() returns.  When ipathinca is the active
    ...
    def add_ipa_wait(self):
        """Dogtag systemd drop-in — not applicable to ipathinca."""
    ...
    def unconfigure_certmonger_renewal_guard(self):
        """Dogtag certmonger guard — not applicable to ipathinca."""

    def reindex_task(self, force=False):
        """Dogtag o=ipaca LDAP reindex — not applicable to ipathinca."""

    def configure_renewal(self):
        """Certmonger renewal tracking for Dogtag NSSDB certs.

        ipathinca manages its own certmonger tracking during install;
    ...
    def setup_acme(self):
        ...
        # Ensure the ipathinca global config singleton is initialised.
        ...
        # invoked, so we load it lazily from the on-disk ipathinca.conf.
    ...
    def is_running(self):
        ...
        try:
            ...
            logger.debug("Could not determine ipathinca service status: %s", e)
            return False
    ...
    def uninstall(self):
        ...
        try:
            ...
            logger.debug("Failed to disable ipathinca service: %s", e)
        ...
        try:
            ...
            logger.debug("Failed to stop ipathinca service: %s", e)
        ...
        try:
            ...
            logger.debug("Failed to mask ipathinca service: %s", e)
        ...
        try:
            ...
            logger.debug("Failed to unmask ipathinca service: %s", e)
        ...
        try:
            ...
            logger.debug("Failed to reset ipathinca service: %s", e)
