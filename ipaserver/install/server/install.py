...
def install_check(installer):
    ...
    if options.setup_kra:
        kra_backend = (
            "ipathinca"
            if getattr(options, 'use_ipathinca', False) else "dogtag"
        )
    ...
    if setup_ca:
        # Determine CA backend: ipathinca or dogtag (default)
        use_ipathinca = getattr(options, 'use_ipathinca', False)
        ca_backend = 'ipathinca' if use_ipathinca else 'dogtag'
...
def install(installer):
    ...
    # Restart krb5kdc when using IPAthinCA to ensure anonymous PKINIT works
    # for webui authentication. This is needed because of timing differences
    # in CA service registration between IPAthinCA and dogtag. Without this
    # restart, anonymous kinit fails with "Pre-authentication failed". Not
    # needed for dogtag.
    if setup_ca and getattr(options, 'use_ipathinca', False):
        service.print_msg("Restarting the KDC")
        krb.restart()
