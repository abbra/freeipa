...
def _upgrade_ipathinca_ca(ca, kra, ds, http, fqdn):
    """ipathinca-specific CA upgrade steps."""
    certificate_renewal_update(ca, kra, ds, http)
    ca.setup_lightweight_ca_key_retrieval()
    ca.setup_acme()
...
def upgrade_configuration():
    ...
    with installutils.stopped_service('pki-tomcatd', 'pki-tomcat'):
        ...
        if ca.is_dogtag_configured():
            ...
            # ipathinca: CLONE suppresses local CRL generation in ipa.conf
    ...
    if ca.is_dogtag_configured():
        ...
        _upgrade_ipathinca_ca(ca, kra, ds, http, fqdn)
