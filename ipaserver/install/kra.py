...
def install_check(api, replica_config, options):
    ...
    # Check if using ipathinca instead of Dogtag
    ...
    if not options.setup_ca:
        if use_ipathinca:
            # ipathinca doesn't use Dogtag, so check for ipathinca CA instead
...
def install(api, replica_config, options, custodia):
    ...
    if use_ipathinca:
        # Use ipathinca KRA (part of ipathinca service, not separate)
        logger.info("Configuring KRA in ipathinca")
        from ipaserver.install.ipathincainstance import IPAThinCAInstance

        # Get hostname same way as Dogtag KRA
        host_name = api.env.host
        ipathinca = IPAThinCAInstance(api.env.realm, host_name)
        ipathinca.enable_kra()

        logger.info("KRA configuration complete")
        return
