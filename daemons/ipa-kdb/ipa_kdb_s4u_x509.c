                               ...
                               krb5_principal *princ_out)
{
    ...
    for (int i = 0; i < sk_GENERAL_NAME_num(sans); i++) {
        ...
        if (!seq || !ASN1_STRING_get0_data(seq) ||
            ASN1_STRING_length(seq) <= 0)
            continue;

        if (parse_krb5_principal_name(ASN1_STRING_get0_data(seq),
                                       (size_t)ASN1_STRING_length(seq),
                                       &pstr) < 0) {
                              ...
                              const unsigned char **data_out, size_t *len_out)
{
    ...
    const X509_EXTENSION *ext = X509_get_ext(cert, idx);
    if (!ext)
        return -1;

    const ASN1_OCTET_STRING *val = X509_EXTENSION_get_data(ext);
    if (!val)
        return -1;
                         ...
                         const ASN1_OCTET_STRING *binding)
{
...

    {
        ...
        if (EVP_DigestVerify(mdctx,
                              ASN1_STRING_get0_data(binding),
                              (size_t)ASN1_STRING_length(binding),
                       ...
                       struct ipadb_e_data **ied_out)
{
    ...
    if (ret != 0) {
        ...
        const X509_NAME_ENTRY *cn_entry = X509_NAME_get_entry(subj, cn_idx);
        const ASN1_STRING *cn_asn1 = X509_NAME_ENTRY_get_data(cn_entry);
        const char *cn_data = (const char *)ASN1_STRING_get0_data(cn_asn1);
        ...
        /* NUL-terminate for krb5_build_principal */
        char cn_buf[256] = { 0 };
        if (cn_len >= (int)sizeof(cn_buf)) {
            krb5_klog_syslog(LOG_ERR,
                             "S4U X.509: Subject CN too long (%d)", cn_len);
            return KRB5KDC_ERR_PREAUTH_FAILED;
        }
        memcpy(cn_buf, cn_data, cn_len);
                        ...
                        krb5_db_entry **entry_out)
{
    ...
    if (authn) {
        if (authn->auth_method) {
            char *m = strndup(
                (const char *)ASN1_STRING_get0_data(authn->auth_method),
                (size_t)ASN1_STRING_length(authn->auth_method));
            if (m) {
        ...
        if (authn->key_fingerprint) {
            ied->s4u->ssh_key_fingerprint = strndup(
                (const char *)ASN1_STRING_get0_data(authn->key_fingerprint),
                (size_t)ASN1_STRING_length(authn->key_fingerprint));
            if (!ied->s4u->ssh_key_fingerprint)
                krb5_klog_syslog(LOG_WARNING,
                                 "S4U X.509: OOM copying key_fingerprint");
        }
        if (authn->client_address) {
            ied->s4u->ssh_client_address = strndup(
                (const char *)ASN1_STRING_get0_data(authn->client_address),
                (size_t)ASN1_STRING_length(authn->client_address));
                                   ...
                                   int fips_mode)
{
    ...
    *entry_out = NULL;
    ...
    if (ASN1_TIME_cmp_time_t(X509_get0_notBefore(cert), now) >= 0 ||
        ASN1_TIME_cmp_time_t(X509_get0_notAfter(cert), now) <= 0) {
        krb5_klog_syslog(LOG_ERR,
