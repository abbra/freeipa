...
static void test_parse_issuer_binding_valid(void **state)
{
    ...
    assert_int_equal(ASN1_STRING_length(ib->binding), 32);
    assert_memory_equal(ASN1_STRING_get0_data(ib->binding), binding, 32);
...
static void test_verify_binding_signature_bad_sig(void **state)
{
    ...
    {
        int blen = ASN1_STRING_length(binding);
        unsigned char *copy = OPENSSL_malloc(blen);
        assert_non_null(copy);
        memcpy(copy, ASN1_STRING_get0_data(binding), blen);
        copy[0] ^= 0xFF;
        ASN1_STRING_set(binding, copy, blen);
        OPENSSL_free(copy);
    }
                             ...
                             int *cert_der_len_out)
{
    ...
    *cert_der_len_out = 0;
    ...
    /* Issuer CN = host principal, Subject CN = user (→ user lookup) */
    {
        ...
        assert_non_null(issuer_name);
        X509_NAME_add_entry_by_NID(issuer_name, NID_commonName, MBSTRING_UTF8,
        ...
        assert_non_null(subj_name);
        X509_NAME_add_entry_by_NID(subj_name, NID_commonName, MBSTRING_UTF8,
                                    (unsigned char *)user, (int)strlen(user),
                                    -1, 0);
        X509_set_subject_name(cert, subj_name);
