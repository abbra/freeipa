...
- Install time: ``IpactaConfig.from_install_params(...)`` stores
  resolved ipacta-native values (signing algorithms, key sizes, …)
  and writes ``ipacta.conf`` via ``write_to_file()``.

- Runtime: ``IpactaConfig.from_file(config_file)`` reads an existing
  ``ipacta.conf`` (replacing the old ``load_config()`` helper).

All PKI-specific knowledge (pki_* key names, PKI ini file paths,
immutability rules) lives in ``ipaserver/install/ipactainstance.py``
...
# Canonical defaults for all ipacta signing parameters.
# Used to initialise IpactaConfig instances and as the fallback when
...
WSGI_DEFAULTS = {
    ...
    "logging": {
        "level": "INFO",
        "log_file": f"{paths.IPACTA_LOG_DIR}/ipacta.log",
...
class IpactaConfig:
    ...
    Install-time signing parameters use ipacta names only — no
    ...
    def __init__(self):
        ...
        self._ocsp_signing_key_size = IPACTA_DEFAULTS[
            "ocsp_signing_key_size"
        ]
        self._audit_signing_algorithm = IPACTA_DEFAULTS[
            "audit_signing_algorithm"
        ]
        self._ecc_curve = IPACTA_DEFAULTS["ecc_curve"]

        # Runtime RawConfigParser from ipacta.conf
    ...
    def from_install_params(
        ...
        ipacta terms (no ``pki_*`` names).  The caller is responsible
        for translating PKI config values before calling this method.

        Parameters left as ``None`` keep the value from
        :data:`IPACTA_DEFAULTS` (set during ``__init__``).

        ``write_to_file()`` must be called later to persist
        ``ipacta.conf`` and populate the ``get()`` pass-through.

        Returns:
            IpactaConfig with install-time attributes set.
        """
        cfg = cls()
        cfg._realm = realm
        cfg._host = host
    ...
    def from_file(cls, config_file=None, strict=True, defaults=None):
        """Create config by reading an existing ``ipacta.conf``.
    ...
    def get(self, section, option, *, fallback=_UNSET):
        ...
        if self._config is None:
            raise RuntimeError(
                "ipacta config not yet loaded from file. "
    ...
    def getint(self, section, option, *, fallback=_UNSET):
        ...
        if self._config is None:
            if fallback is not _UNSET:
                return fallback
            raise RuntimeError("ipacta config not yet loaded from file.")
        if fallback is not _UNSET:
    ...
    # -----------------------------------------------------------------------
    # Write resolved config to ipacta.conf (install-time)
    # -----------------------------------------------------------------------

    def write_to_file(self, config_file):
        """Write ``ipacta.conf`` from the stored install-time parameters.

        Uses the signing algorithm and key-size values stored by
        ``from_install_params()``, substitutes them into the
        ``ipacta.conf.template``, writes the result, then reads it
        ...
        ipautil.copy_template_file(
            Path(paths.USR_SHARE_IPA_DIR) / "ipacta.conf.template",
            config_file,
            dict(
                REALM=self._realm,
                DOMAIN=self._domain,
                ...
                IPACTA_PID=paths.IPACTA_PID,
                IPACTA_CERTS_DIR=paths.IPACTA_CERTS_DIR,
                IPACTA_PRIVATE_DIR=paths.IPACTA_PRIVATE_DIR,
                IPACTA_LOG_DIR=paths.IPACTA_LOG_DIR,
                # Signing algorithms
