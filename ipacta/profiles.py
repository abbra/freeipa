...
from ipacta.profile_monitor import ProfileChangeMonitor
...
class ProfileManager:
    ...
    def __init__(
        ...
        Args:
            ...
            config: ConfigParser object with Ipacta configuration (optional)
    ...
    def store_all_profiles_to_ldap(self):
        ...
        Installs only the profiles needed for Ipacta operation.
    ...
    def _get_variable_context(self) -> dict:
        ...
        """
        # realm and domain must be present in the ipacta config file
        if not self.config:
            raise RuntimeError(
                "Cannot load profiles: no configuration available. "
                "ProfileManager requires an ipacta config with "
    ...
    def get_extensions_for_profile(self, profile_id: str):
        ...
        import synta.ext
        import synta.oids
        from ipacta.profile_defaults import (
