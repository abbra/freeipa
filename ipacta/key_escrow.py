...
from ipacta.key_encryption import KeyEncryption
...
class PythonKeyEscrowBackend:
    ...
    def archive_encrypted_data(
        ...
        """
        logger.debug("Archiving data for client key ID: %s", client_key_id)
    ...
    def list_keys(
        ...
        """
        logger.debug(
            "Listing keys for client: %s, status: %s", client_key_id, status
        )
    ...
    def modify_key_status(self, key_id: str, new_status: str):
        ...
        """
        logger.debug("Modifying key status: %s -> %s", key_id, new_status)
    ...
    def retrieve_key(self, key_id: str, wrapped_session_key: bytes) -> KeyData:
        ...
        """
        logger.debug("Retrieving key: %s", key_id)
        ...
        for filename in os.listdir(self.storage_path):
            ...
            for record in records:
                if record["key_id"] == key_id:
                    if record["status"] != KEY_STATUS_ACTIVE:
                        # TODO: errors.InvalidRequest would be good to have
                    ...
                    logger.info("Successfully retrieved key: %s", key_id)
