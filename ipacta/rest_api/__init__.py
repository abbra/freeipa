...
"""REST API package for Ipacta — Flask blueprint-based implementation."""
...
from ipacta.rest_api_helpers import error_response
...
for _mod_info in pkgutil.iter_modules(__path__):  # noqa: F821
    if _mod_info.name.startswith("_"):
        continue
    _mod = importlib.import_module(f"ipacta.rest_api.{_mod_info.name}")
    if hasattr(_mod, "bp"):
...
def create_app(config=None):
    ...
    if config:
        if "config" in config:
            ...
            logging.getLogger("ipacta").setLevel(log_level)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Ipacta REST API Server")
...
# Re-export for wsgi.py compat: from ipacta.rest_api import ca_backend
