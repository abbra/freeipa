    ...
    try:
        ...
        for subca in subcas:
            ca_info = {
                ...
                "externalKey": subca.external_key,
                "ready": not subca.external_key,
            }
            if subca.ca_cert:
...
def create_authority():
    ...
    Request body should contain:
        ...
        - csrData: PEM-encoded PKCS#10 CSR (optional).  When present, the
                   private key is NOT generated by IPAThinCA; the CSR is signed
                   by the parent CA instead.  The returned authority will have
                   externalKey=true and ready=false.  csrData is input-only and
                   is never stored or returned in GET responses.
        - profileId: Signing profile identifier (optional, input-only).
                     Accepted for Dogtag API compatibility; currently reserved
                     for future use.
    """
    try:
        ...
        # External-key fields — input-only, never stored or returned in GET
        csr_pem = data.get("csrData")
        profile_id = data.get("profileId")

        ca_backend = get_python_ca_backend()

        # Create sub-CA (external-key path when csrData is present)
        ...
        # csrData and profileId are input-only: never returned in responses.
        authority_info = {
            "id": subca.ca_id,
            "dn": subca.subject_dn,
            "issuerDN": issuer_dn,
