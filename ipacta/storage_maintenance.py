...
class MaintenanceStorage(BaseStorageBackend):
    ...
    def cleanup_old_requests(self, days: int = 90):
        ...
        with self._get_ldap_connection() as ldap:
            ...
            logger.debug(
                "Cleaning up requests older than %s (Dogtag schema)",
            ...
            try:
                ...
                logger.debug(
                    "Cleaned up %d old requests (Dogtag schema)", deleted_count
                )
    ...
    def delete_old_certificates(
        ...
        with get_ldap_connection() as conn:
            ...
            try:
                ...
                for entry in entries:
                    try:
                        ...
                        if deleted_count % 100 == 0:
                            logger.info(
                                "Pruning progress: %d certificates deleted",
                                deleted_count,
                            )
                    except Exception as e:
                        logger.error(
                            "Failed to delete certificate %s: %s", entry.dn, e
                        )

                logger.info(
                    "Certificate pruning complete: %d deleted", deleted_count
                )
                ...
                logger.error(
                    "Error during certificate pruning: %s", e, exc_info=True
                )
                raise
            ...
            try:
                ...
                for entry in entries:
                    try:
                        ...
                        if deleted_count % 100 == 0:
                            logger.info(
                                "Pruning progress: %d requests deleted",
                                deleted_count,
                            )
                    except Exception as e:
                        logger.error(
                            "Failed to delete request %s: %s", entry.dn, e
                        )

                logger.info(
                    "Request pruning complete: %d deleted", deleted_count
                )
                ...
                logger.error(
                    "Error during request pruning: %s", e, exc_info=True
                )
                raise
