...
Example:
    >>> from ipacta.certificate_lifecycle import CertificateLifecycle, \
    >>>     CertificateEvent
    >>>
...
from ipacta.exceptions import InvalidStateTransition
...
# InvalidStateTransition is now imported from ipacta.exceptions
...
class CertificateLifecycle:
    ...
    def __init__(
        ...
        logger.debug(
            "Initialized lifecycle for cert %s in state %s",
            serial_number,
            initial_state.value,
        )
    ...
    def transition(
        ...
        logger.debug(
            "Certificate %s state transition: %s",
            self.serial_number,
            transition,
        )
...
try:
    ...
    logger.error("State machine validation failed: %s", e)
    raise
