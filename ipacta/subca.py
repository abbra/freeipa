...
from ipacta.ca import PythonCA, CertificateRequest, CertificateRecord
...
class SubCAManager:
    ...
    def initialize_ldap_schema(self):
        ...
        with self._get_ldap_connection() as ldap:
            ...
            # This ACI allows the ipacta service (running as ipacasrv) to
    ...
    def list_subcas(self) -> List[SubCA]:
        ...
        with self._get_ldap_connection() as ldap:
            ...
            try:
                # Only query sub-CAs that have the ipactaca objectClass
                # This filters out old Dogtag sub-CAs that ipacta cannot
                # load
                # OPTIMIZED: Only fetch ipaCaId for listing sub-CAs
                entries = ldap.get_entries(
                    self.ldap_base_dn,
                    scope=ldap.SCOPE_ONELEVEL,
                    filter="(&(objectClass=ipaca)(objectClass=ipactaca))",
