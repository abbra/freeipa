...
from ipacta.storage_base import BaseStorageBackend
from ipalib import errors
from ipapython.dn import DN
from ipacta import x509_utils
from ipacta.certificate_types import (
    CertificateRecord,
    RevocationReason,
    CertificateRequest,
)
from ipacta.storage_base import biginteger_to_db

# Import LDAP filter escaping from shared location
from ipacta.storage_base import escape_filter_chars
...
class CertificateStorage(BaseStorageBackend):
    ...
    def store_certificate(self, cert_record, allow_update=True):
        ...
        with self._get_ldap_connection() as ldap:
            ...
            try:
                ...
                # Map ipacta status to Dogtag status
    ...
    def _map_status_to_dogtag(self, status: str) -> str:
        """Map ipacta status values to Dogtag status values"""
    ...
    def _map_status_from_dogtag(self, status: str) -> str:
        """Map Dogtag status values to ipacta status values"""
        status_map = {
            "VALID": "VALID",
            "REVOKED": "REVOKED",
            "EXPIRED": "EXPIRED",
    ...
    def get_certificate(self, serial_number: int):
        ...
        with self._get_ldap_connection() as ldap:
            ...
            try:
                ...
                # Map Dogtag status to ipacta status
    ...
    def get_revoked_certificates(
        ...
        """Get revoked certificates for CRL generation.
    ...
    def get_revoked_for_crl(self):
        ...
        with self._get_ldap_connection() as ldap:
            ...
            for entry in entries:
                # Serial number stored as plain decimal in cn
                ...
                # Revocation reason (integer)
                reason_int = 0
                rev_raw = entry.get("revinfo", entry.get("revInfo", [None]))[0]
    ...
    def store_request(self, cert_request):
        ...
        with self._get_ldap_connection() as ldap:
            ...
            csr_pem = (
                synta.CertificationRequest.to_pem(csr).decode("ascii")
                if csr is not None
                else ""
            )
