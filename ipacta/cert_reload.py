...
"""
Certificate reload functionality for ipacta
...
class CertificateReloadManager:
    ...
    def reload_certificates(self) -> dict:
        ...
        with self._reload_lock:
            ...
            try:
                ...
                logger.info(
                    "Certificate reload completed: %s", results["status"]
                )
                reloaded = (
                ...
                logger.info("Reloaded: %s", reloaded)

                if results["errors"]:
                    logger.error(
                        "Errors during reload: %s",
                        "; ".join(results["errors"]),
                    )

            except Exception as e:
                logger.error("Certificate reload failed: %s", e, exc_info=True)
    ...
    def _reload_ca_cert(self) -> dict:
        ...
        try:
            ...
            logger.debug("Loading CA certificate from %s", ca_cert_path)
            ...
            if old_serial != new_serial:
                logger.debug(
                    "Reloading CA certificat with serial number "
                    "change: %s -> %s",
                    old_serial,
                    new_serial,
                )
            ...
            logger.info(
                "CA certificate reloaded successfully (serial changed from "
                "%s to %s)",
                old_serial,
                new_serial,
            )
    ...
    def reload_server_ssl_cert(self, cert_path: str, key_path: str) -> dict:
        ...
        """
        try:
            logger.debug("Validating server SSL certificate at %s", cert_path)
            ...
            logger.info(
                "Server SSL certificate validated successfully (serial: %s)",
                cert.serial_number,
            )
