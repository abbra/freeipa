...
Prerequisites:
    - ipacta.service running on the test host
    - IPA CA configured and operational

Run with:
    pytest ipacta/tests/test_threading.py -v
"""
...
def _service_is_active():
    ...
    try:
        result = subprocess.run(
            ["systemctl", "is-active", "ipacta.service"],
            capture_output=True,
            text=True,
            timeout=5,
            check=False,
...
pytestmark = pytest.mark.skipif(
    ...
    reason="ipacta.service not active",
)
...
def _curl_get_json(
    base_url, path, timeout=30, client_cert=None, client_key=None
):
...
def _curl_post_json(
    base_url, path, payload, timeout=30, client_cert=None, client_key=None
):
...
class TestConcurrentRequests:
    ...
    def test_concurrent_info_requests(self, service_info):
        ...
        assert len(connected) > 0, f"All requests failed to connect: {counts}"
    ...
    def test_concurrent_mixed_endpoints(self, service_info):
        ...
        assert len(results) == n_total, (
            f"Expected {n_total} results, got {len(results)} "
            "— possible deadlock"
        )
    ...
    def test_high_concurrency_burst(self, service_info):
        ...
        def fetch():
            try:
                code = _curl_get(service_info["base_url"], "/pki/rest/info")
                results.append(code)
            except Exception as e:
                errors.append(e)
...
class TestConcurrentConflicts:
    ...
    def test_concurrent_info_responses_consistent(self, service_info):
        """Concurrent /pki/rest/info requests must return identical data."""
        results = []
        errors = []
        ...
        assert len(ok_results) >= 2, (
            ...
            f"{len(ok_results)}. "
            f"Status codes: {Counter(r[0] for r in results)}"
        )
    ...
    def test_concurrent_profile_list_consistent(self, service_info):
        ...
        for _, data in ok_results:
            if isinstance(data, dict) and "entries" in data:
                ...
                ids = sorted(e.get("profileId", e.get("id", "")) for e in data)
            else:
                continue
    ...
    def test_concurrent_cert_requests_unique_ids(self, service_info):
        ...
        # Response format: {"entries": [{"requestId": "...",
        #                                "certId": "0x..."}]}
        request_ids = []
        cert_ids = []
        ...
        assert len(request_ids) == len(set(request_ids)), (
            ...
            f"IDs: {request_ids}, "
            "Duplicates: "
            f"{[x for x in request_ids if request_ids.count(x) > 1]}"
        )
    ...
    def test_concurrent_read_same_cert(self, service_info):
        """Concurrent reads of the same cert must return identical data."""
        ra_cert = service_info.get("ra_cert")
        ra_key = service_info.get("ra_key")
        if not ra_cert or not ra_key:
            pytest.skip("RA agent credentials not available")
        ...
        with concurrent.futures.ThreadPoolExecutor(
            ...
            futures = [
                pool.submit(fetch) for _ in range(CONFLICT_READ_CERT_REQUESTS)
            ]
            concurrent.futures.wait(futures)
    ...
    def test_mixed_read_write_no_corruption(self, service_info):
        ...
        req_ids = []
        for _status, data in results_submit:
            if data is None:
                continue
            entries = data.get(
                "entries", [data] if "requestId" in data else []
...
class TestHeavyLoad:
    ...
    def test_sustained_mixed_heavy_load(self, service_info):
        ...
        with concurrent.futures.ThreadPoolExecutor(
            ...
            futures = []
            futures.extend(
                pool.submit(fetch_info) for _ in range(HEAVY_MIXED_INFO_READS)
            )
            futures.extend(
        ...
        all_statuses = [r[0] for r in results_read] + [
            r[0] for r in results_submit
        ]
