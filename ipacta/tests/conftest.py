...
"""
Shared test infrastructure for ipacta tests
...
def ipacta_config():
    """Initialize ipacta global config for tests that need it.

    Reads /etc/ipa/ipacta.conf if available, otherwise builds a
    minimal config from defaults.
    """
    import ipacta

    conf_path = Path("/etc/ipa/ipacta.conf")
...
@pytest.fixture
def variable_context(ipacta_config):
...
@pytest.fixture
def sample_csr(ipacta_config, sample_key):
...
def get_base_url():
    """Get the ipacta service base URL from config."""
