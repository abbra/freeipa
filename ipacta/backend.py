...
This module is part of the ipacta service and HAS ACCESS TO CA PRIVATE
KEYS.
...
2. ipacta.service (ipaca) -> rest_api.py -> backend.py (THIS FILE) -> CA
   private keys

The backend is accessed ONLY through the REST API (rest_api.py) which runs in
the ipacta service (systemd service running as ipaca with access to
private keys).
...
from ipacta.exceptions import InvalidCAConfiguration, ProfileNotFound
from ipacta.profiles import ProfileManager
from ipacta.pruning import PruningManager
...
class PythonCABackend:
    ...
    def __init__(self, config=None):
        ...
        # Set global config for all ipacta components
        ...
        self.ca = InternalCA(
            ...
            ca_key_path=paths.IPACTA_SIGNING_KEY,
            ca_id=ca_id,
    ...
    def _validate_hostname_config(self):
        ...
        This prevents misconfiguration where the hostname in ipacta.conf
    ...
    def update_crl(self) -> dict:
        ...
        1. ipacta directory (for REST API access)
        ...
        try:
            ...
            # Store CRL in ipacta directory (for REST API)
    ...
    def process_acme_request(
        ...
        Args:
            ...
            account_key: JWK dict for the account key (from jwk header field).
        ...
        try:
            if endpoint == "new-account":
                ...
                # Ownership is verified inside revoke_certificate():
                # the requesting account must own the order that issued it.
                ...
                try:
                    # RFC 8555 uses base64url without padding
