...
"""Certificate generation helper for IpactaInstance.
...
class Certs:
    ...
    def _configure_ca_certs(self):
        ...
        if self.ca_signing_cert and self.ca_signing_key:
            ...
            logger.debug(
                "CA certificate and key already exist in ipacta directories"
            )
            ...
            # Replica install: CA cert+key were imported into the NSSDB by
            # _import_replica_keys() above.  Extract the CA cert to the
            # working PEM path so the rest of the install can proceed.
            ...
            if not nssdb.cert_exists(ca_nickname):
                raise RuntimeError(
                    ...
                    "key import — check that keys were transferred from master"
                )
            ca_cert = nssdb.extract_certificate(ca_nickname)
            ca_cert_pem = synta.Certificate.to_pem(ca_cert)
    ...
    def _generate_external_ca_csr(self):
        ...
        state_file = "/var/lib/ipa/ipacta_external_ca.state"
        state = {
            "step": 1,
    ...
    def _install_external_ca_cert(self):
        ...
        # Export cert and key to PEM for Ipacta runtime
    ...
    def _store_hsm_configuration(self):
        ...
        try:
            ...
            # The IPA CA LDAP entry must exist before HSM config can be stored.
            # There is no deferred-storage code path; fail loudly so the
            # installation step ordering is fixed rather than silently losing
            # the HSM configuration (which would break replicas).
    ...
    def _generate_subsystem_certs(self):
        """Generate PKI subsystem certificates through ipacta CA.
        ...
        """
        logger.debug(
            "Generating PKI subsystem certificates through ipacta CA "
        ...
        logger.debug(
            "All PKI subsystem certificates generated through ipacta CA"
        )
    ...
    def _create_pkidbuser_entry(self, subsystem_cert):
        ...
        entry = self.ldap.make_entry(
            ...
            description=[
                "CA database user - created by Ipacta for compatibility"
            ],
        )
    ...
    def _generate_server_cert(self):
        ...
        """
        logger.debug(
            "Generating server SSL certificate through ipacta CA "
            "(NSSDB + PEM file for gunicorn)"
        )

        server_cert_path = self.ipaca_certs_dir / "server.crt"
        server_key_path = Path(paths.IPACTA_DIR) / "private" / "server.key"
        ...
        if nssdb.cert_exists(server_nickname):
            ...
            if not server_cert_path.exists() or not server_key_path.exists():
                ...
                with open(server_cert_path, "wb") as f:
                    f.write(synta.Certificate.to_pem(cert))
    ...
    def _generate_ra_cert(self):
        """Generate RA agent certificate through ipacta CA.

        Uses the same issuance flow as subsystem certificates for consistency
        and automatic LDAP storage.
        """
        logger.debug("Generating RA agent certificate through ipacta CA")
