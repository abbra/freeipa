...
"""NSS database helper for IpactaInstance.
...
class NSSDB:
    ...
    def create_nssdb(self):
        ...
        Creates NSSDB at /etc/pki/pki-tomcat/alias/ to ensure ipacta is a
        ...
        - Smooth migration from Dogtag to ipacta
        """
        ...
        if existing_files:
            logger.info(
                "Removing existing NSSDB files from previous installation: %s",
                existing_files,
            )
        ...
        try:
            ...
            logger.debug("NSSDB created successfully at %s", self.nssdb_dir)
        finally:
    ...
    def import_cert_to_nssdb(
        ...
        """
        logger.debug("Importing certificate '%s' to NSSDB", nickname)
        ...
        try:
            ...
            logger.debug(
                "Creating PKCS#12 from %s and %s", cert_pem_path, key_pem_path
            )
            ipautil.run(
                [
                    "openssl",
            ...
            logger.debug(
                "Importing PKCS#12 to NSSDB with nickname '%s'", nickname
            )
            ipautil.run(
                [
                    "pk12util",
            ...
            if trust_flags != "u,u,u":
                logger.debug(
                    "Setting trust flags to '%s' for '%s'",
                    trust_flags,
                    nickname,
                )
                ipautil.run(
            ...
            if result.returncode == 0:
                logger.debug(
                    "Certificate '%s' successfully imported to NSSDB", nickname
                )
            else:
                raise RuntimeError(
