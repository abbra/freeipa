...
"""LDAP setup helper for IpactaInstance.
...
class LDAPSetup:
    ...
    def _install_dogtag_schema(self):
        ...
        logger.debug("Using LDAP socket: %s", socket_url)
        logger.debug("Loading Dogtag schema from %s", schema_file)
        ...
        if result.returncode == 0:
            ...
            logger.error("Stdout: %s", result.output)
            logger.error("Error output: %s", result.error_output)
            logger.error("Raw error: %s", result.raw_error_output)
    ...
    def _create_ds_db(self):
        ...
        try:
            ...
            logger.debug("LDAP backend %s already exists", backend)
        except errors.NotFound:
            # Create new backend
            logger.debug("Creating LDAP backend: %s", backend)
            ...
            logger.debug("LDAP backend %s created successfully", backend)
        ...
        try:
            ...
            self.ldap.get_entry(dn)
            logger.debug("Mapping tree for %s already exists", suffix)
        except errors.NotFound:
            # Create new mapping tree
            logger.debug("Creating mapping tree for suffix: %s", suffix)
            entry = self.ldap.make_entry(
                dn,
            ...
            self.ldap.add_entry(entry)
            logger.debug("Mapping tree for %s created successfully", suffix)
    ...
    def _initialize_ldap_storage(self):
        ...
        logger.debug("Initializing LDAP storage for ipacta")
        ...
        try:
            ...
            logger.error(
                "Failed to initialize Dogtag LDAP schema: %s", e, exc_info=True
            )
            raise RuntimeError(f"Cannot initialize ipacta LDAP schema: {e}")
    ...
    def _configure_ldapi_autobind(self):
        ...
        This enables the ipacta service (running as Unix user ipaca) to
...
"""
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                logger.warning(
                    "Failed to enable LDAPI autobind mapping: %s", error_msg
                )
    ...
    def _import_profiles_ldap(self):
        ...
        # This is separate from our ipacta storage at
        ...
        try:
            ...
            logger.debug(
                "Creating certprofiles container: %s", certprofiles_base
            )
            ...
            try:
                ...
                logger.debug("Creating ca container: %s", ca_base)
        ...
        for profile_entry in dogtag.INCLUDED_PROFILES:
            ...
            try:
                ...
                logger.warning(
                    "Failed to load profile %s: %s, skipping", profile_id, e
                )
                continue
            ...
            try:
                ...
                logger.debug("Profile %s already exists in LDAP", profile_id)
            except errors.NotFound:
                # Create profile entry in LDAP
                logger.debug("Creating profile %s in LDAP", profile_id)
        ...
        logger.debug(
            "Imported %s certificate profiles to IPA tree",
            len(dogtag.INCLUDED_PROFILES),
        )
        ...
        try:
            ...
            logger.error(
                "Failed to import certificate profiles to LDAP: %s",
                e,
                exc_info=True,
            )
            raise RuntimeError(
