...
"""
ACME LDAP Database Backend for ipacta
...
from ipacta.ldap_utils import get_ldap_connection
from ipacta.storage_base import LDAPStorageMixin, escape_filter_chars
...
class ACMEStorageBackend(LDAPStorageMixin):
    ...
    def __init__(self, config):
        ...
        Args:
            config: RawConfigParser from ipacta.conf
        """
        self.config = config
    ...
    def get_account_by_jwk(self, jwk: Dict) -> Optional[Dict]:
        ...
        """
        try:
            from ipacta.jwk import JWK as ACMEJWKClass
    ...
    def find_order_for_certificate(self, cert_der: bytes) -> Optional[Dict]:
        ...
        Returns the order dict (same shape as get_order()) or None if not found.
        """
        try:
            with get_ldap_connection() as conn:
                # Find all completed orders (those with acmeCertificateId set)
                entries = conn.get_entries(
                    self.orders_dn,
                    scope=conn.SCOPE_ONELEVEL,
                    filter="(&(objectClass=acmeOrder)(acmeCertificateId=*))",
                    attrs_list=["acmeOrderId", "acmeAccountId",
                                "acmeCertificateId"],
                )
        except Exception as e:
        ...
        for entry in entries:
            ...
            if not stored_pem:
                continue
            try:
                stored_cert = synta.Certificate.from_pem(
                    stored_pem.encode()
                )
                if stored_cert.to_der() == cert_der:
