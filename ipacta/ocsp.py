...
class OCSPResponder:
    ...
    def _generate_ocsp_signing_cert(self):
        ...
        try:
            ...
            if self.ocsp_key_path:
                ...
                # Use O_CREAT|O_EXCL|O_WRONLY with mode at creation to avoid
    ...
    def create_response(self, request_der: bytes) -> bytes:
        ...
        try:
            ...
            # check and removal are atomic (M-7).
            ...
            # we return internalError (status=2) per RFC 6960 §2.3 rather
            # than UNKNOWN, which TLS stacks may treat as good.
            try:
                (
                    cert_status,
                    revocation_time,
                ...
                return self._create_error_response(status=2)
...
class OCSPResponderManager:
    ...
    def __init__(self, base_storage_path: str = None):
        ...
        self.base_storage_path = Path(
            base_storage_path or f"{paths.IPACTA_DIR}ocsp"
        )
