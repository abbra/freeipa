...
class CRLStorage(BaseStorageBackend):
    ...
    def store_crl(
        ...
        with self._get_ldap_connection() as ldap:
            ...
            try:
                ...
                logger.debug("Updated CRL issuing point: %s", crl_name)

            except errors.NotFound:
                # Create new CRL issuing point
                logger.debug("Creating CRL issuing point: %s", crl_name)
                ...
                logger.debug("Created CRL issuing point: %s", crl_name)
    ...
    def get_crl(self, crl_name: str = "MasterCRL") -> Any:
        ...
        with self._get_ldap_connection() as ldap:
            ...
            try:
                ...
                logger.warning(
                    "CRL %s found but no CRL data attribute present", crl_name
                )
                return None

            except errors.NotFound:
                logger.debug("CRL issuing point %s not found", crl_name)
                return None
    ...
    def get_crl_info(self, crl_name: str = "MasterCRL") -> Any:
        ...
        if (
            ...
            logger.debug(
                "Returning cached CRL info for %s (TTL cache hit)", crl_name
            )
        ...
        with self._get_ldap_connection() as ldap:
            ...
            try:
                ...
                if self._crl_info_cache is not None:
                    ...
                    logger.debug("Cached CRL info for %s (TTL=300s)", crl_name)
    ...
    def delete_crl_issuing_point(self, crl_name: str):
        ...
        with self._get_ldap_connection() as ldap:
            ...
            try:
                ...
                logger.info("Deleted CRL issuing point: %s", crl_name)
            except errors.NotFound:
                logger.warning(
                    "CRL issuing point %s not found for deletion", crl_name
                )
