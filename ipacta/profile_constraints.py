...
from ipacta.profile import Constraint
...
class SubjectNameConstraint(Constraint):
    ...
    def validate(self, csr, context: dict) -> List[str]:
        ...
        if "final_subject_dn" in context:
            ...
            logger.debug(
                "SubjectNameConstraint validating final subject: %s",
                subject_dn,
            )
        else:
            ...
            logger.debug(
                "SubjectNameConstraint validating CSR subject: %s", subject_dn
            )
...
def create_constraint(class_id: str, params: Dict[str, Any]) -> Constraint:
    ...
    if not constraint_class:
        logger.warning(
            "Unknown constraint class '%s', using NoConstraint", class_id
        )
        return NoConstraint()
    ...
    try:
        ...
        logger.error("Failed to create constraint %s: %s", class_id, e)
        raise
