...
"""
Gunicorn configuration for ipathinca.

Loaded via --config python:ipathinca.gunicorn_conf when ipathinca is started
...
def _start_rate_limiter_purge(worker_pid):
    ...
    """
    from ipathinca import rate_limit as _rl
...
def post_fork(server, worker):
    ...
    try:
        from ipathinca.ldap_utils import close_ldap_pool, close_ldap_connection
        close_ldap_pool()
...
def when_ready(server):
    ...
    $NOTIFY_SOCKET so that ``systemctl start ipathinca.service`` returns
