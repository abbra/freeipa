...
def kra_info():
    ...
    try:
        ...
        if kra_backend is None:
            detail = f"; init error: {kra_init_error}" if kra_init_error else ""
            return error_response(
                "KRANotAvailable",
...
def kra_status():
    ...
    try:
        ...
        if kra_backend is None:
            detail = f"; init_error={kra_init_error}" if kra_init_error else ""
            return Response(
...
def acme_directory():
    ...
    try:
        ...
        if not ca_backend.acme_state.is_enabled():
            return Response(
                "ACME service is disabled", status=503, mimetype="text/plain"
            )

        directory = ca_backend.get_acme_directory()
...
def acme_new_nonce():
    ...
    try:
        ...
        nonce = ca_backend.acme_server.generate_nonce()
...
@app.route("/acme/<path:endpoint>", methods=["POST"])
...
def acme_enable():
    ...
    try:
        init_ca()
        ca_backend.acme_state.set_enabled(True)
...
def acme_disable():
    ...
    try:
        init_ca()
        ca_backend.acme_state.set_enabled(False)
...
def get_hsm_config():
    ...
    try:
        ...
        if (
            ...
            hsm_config = ca_backend.ca.ldap_storage.get_hsm_config(ca_id)
...
def update_hsm_config():
    ...
    try:
        ...
        if (
            ...
            ca_backend.ca.ldap_storage.store_hsm_config(ca_id, data)
...
def delete_hsm_config():
    ...
    try:
        ...
        if (
            not hasattr(ca_backend.ca, "ldap_storage")
            or not ca_backend.ca.ldap_storage
        ):
            return error_response(
                "NotSupported", "LDAP storage not available", 500
            )
        ...
        hsm_config_dict = ca_backend.ca.ldap_storage.get_hsm_config(ca_id)
        ...
        if hasattr(ca_backend.ca.ldap_storage, "delete_hsm_config"):
            ca_backend.ca.ldap_storage.delete_hsm_config(ca_id)
            return success_response(
                {
                    "Status": "SUCCESS",
            ...
            ca_backend.ca.ldap_storage.store_hsm_config(ca_id, hsm_config_dict)
            return success_response(
                {
                    "Status": "SUCCESS",
